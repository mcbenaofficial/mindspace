...
Check for `.claude/propagation.json`. If present, this repo is one of several sibling repos that together form one product (`repos` lists the full set), and these rules apply on top of everything above:

- **When a request completed here must also be built out in sibling repos**, add a `## Propagation` section to its request file — one row per applicable sibling (omit siblings the feature genuinely doesn't apply to, or mark `N/A — <reason>`):
  ...
  | riserve_backend | Pending | — |
  ```

  Statuses: `Pending` → `In build` → `Landed`, or `N/A — <reason>`. Update a row to `Landed` (linking the downstream request in the third column) only when that sibling's request file is Complete.

- **When a plan here builds out a feature that originated in a sibling repo**, its header block MUST carry an `- **Origin:** <sibling-repo>/docs/requests/<file>.md` line. The propagation matrix cross-references this line — without it the build is invisible to the product-level view. Work native to this repo simply omits the line.

The cross-repo matrix is generated by `propagation-status.py` in the harness kit (run via `report-all.sh`); it flags features declared Landed without a verified downstream request, completed features with no downstream activity after 14 days, and Origin lines pointing at missing files. Repos without a `propagation.json` (e.g. internal sandboxes like riserve-v9, or unrelated projects) are not part of any matrix — do not add Propagation sections or Origin lines there.
